...
// - Agrega â€œdashboardâ€ superior: tarjetas Hoy/Programados/Todos/Con indicador/Terminados.
// - Agrega â€œLista sugeridaâ€ + â€œMis listasâ€ (RaÃ­z + Carpetas + Papelera).
...
class _StartPageState extends State<StartPage> {
  ...
  static const String _kBuildSha = String.fromEnvironment('GIT_SHA', defaultValue: 'dev');
  static const String _kBuildTime = String.fromEnvironment('BUILD_TIME', defaultValue: '');
  ...
  bool _looksLikeHttpUrl(String s) {
    var v = s.trim();
    if (v.isEmpty) return true; // permitir â€œsin configurarâ€
  ...
  Future<void> _purgeExpiredTrashIfNeeded() async {
    // FIX CRÃTICO: si falla SheetStore.delete, NO removemos metadata.
    ...
    for (final id in expiredIds) {
      try {
        ...
        // Se mantiene en papelera para no â€œrevivirâ€ datos inconsistentes.
      }
    }
  ...
  Future<void> _newSheet() async {
    ...
    // createdAt real (fecha de creaciÃ³n)
    _sheetCreatedAtMs[id] = DateTime.now().millisecondsSinceEpoch;

    // Si estÃ¡s parado en una carpeta (tab sheets), la planilla nace ahÃ­
  ...
  Future<void> _open(SheetMeta m) async {
    ...
    if (_trashDeletedAtMs.containsKey(m.id)) {
      final ok = await _confirmCupertino(
        ...
        message: 'Para abrir y editar, primero hay que restaurar la planilla. Â¿Restaurar ahora?',
        okText: 'Restaurar',
      );
      if (ok != true) return;
  ...
  Future<void> _moveToTrash(SheetMeta m) async {
    ...
    final ok = await _confirmCupertino(
      ...
      message: 'Se podrÃ¡ recuperar durante $_trashTtlDays dÃ­as. Â¿QuerÃ©s continuar?',
      okText: 'Mover',
      danger: true,
    );
  ...
  Future<void> _editNote(SheetMeta m) async {
    ...
    final result = await _promptMultilineCupertino(
      ...
      placeholder: 'Ej: â€œEnviar a cliente hoy 18:00â€ / â€œWP: revisar mediciÃ³n 3â€',
      okText: 'Guardar',
  ...
  Future<void> _deleteFolderFlow(BuildContext ctx, _Folder folder) async {
    final ok = await _confirmCupertino(
      ...
      message: 'Las planillas vuelven a â€œRaÃ­zâ€. Â¿Eliminar â€œ${folder.name}â€?',
      okText: 'Eliminar',
      danger: true,
    );
    if (ok != true) return;
  ...
  Future<_Folder?> _createFolderDialog(BuildContext ctx) async {
    ...
    final name = await _promptTextCupertino(
      ...
      info: _PromptInfo(
        ...
        message:
        'Ejemplos: â€œ$suggestedâ€, â€œSeptiembre 2026â€, â€œObra Xâ€. Un solo nivel, simple y ordenado.',
      ),
    );
                ...
                child: Column(
                  children: [
                    ...
                    _CupertinoToggleRow(
                      ...
                      subtitle: 'Activa la automatizaciÃ³n cuando tu producto lo ejecute.',
                      value: autoSend,
        ...
        return CupertinoActionSheet(
          ...
          actions: [
            ...
            CupertinoActionSheetAction(
              ...
              child: Text(_tab == _HomeTab.sheets ? 'Ir a Papelera' : 'Ir a Planillas'),
            ),
          ],
          cancelButton: CupertinoActionSheetAction(
            onPressed: () => Navigator.of(ctx).pop(),
  ...
  List<SheetMeta> get _visibleSheets {
    ...
    if (_tab == _HomeTab.sheets && _quick != _QuickFilter.none) {
      ...
      if (_quick == _QuickFilter.today) {
        ...
        list = list.where((m) => (_sheetNotes[m.id] ?? '').trim().isNotEmpty).toList();
      }
    }
  ...
  Widget build(BuildContext context) {
    ...
    final scheduledCount = 0; // placeholder intencional (sin feature de agenda por ahora)
                ...
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: _RemindersSummaryGrid(
                      ...
                      onTapScheduled: () => _applySummaryTap(_SummaryKind.scheduled),
                      onTapAll: () => _applySummaryTap(_SummaryKind.all),
                      onTapFlagged: () => _applySummaryTap(_SummaryKind.flagged),
                      onTapCompleted: () => _applySummaryTap(_SummaryKind.completed),
                ...
                SliverToBoxAdapter(
                  child: Padding(
                    ...
                    child: _SuggestedListCard(
                      ...
                      subtitle: 'Organiza automÃ¡ticamente las planillas nuevas.',
                      onAdd: () async {
                        final created = await _createFolderDialog(context);
                        if (created == null) return;
                        if (!mounted) return;
                          ...
                          onTap: (item) {
                            switch (item.kind) {
                              ...
                              case _ListKind.folder:
                                _applyListTap(_ListKind.folder, folderId: item.folderId);
                                break;
                        ...
                        child: Column(
                          ...
                          children: [
                            CupertinoSearchTextField(
                              ...
                              placeholder: 'Buscar por tÃ­tulo o mensaje destacadoâ€¦',
                            ),
                  ...
                  child: Padding(
                    ...
                    child: Text(
                      _tab == _HomeTab.trash
                          ...
                          : 'Mostrando ${data.length} en â€œ${_folderName(_selectedFolderId)}â€'),
                      style: TextStyle(
                        ...
                        children: [
                          for (int i = 0; i < data.length; i++)
                            _AppleSheetRow(
                              ...
                              daysLeftInTrash:
                              _tab == _HomeTab.trash ? _daysLeftInTrash(data[i].id) : null,
                              onOpen: () => _open(data[i]),
  ...
  List<_HomeListItem> _buildHomeLists(_ApplePalette colors) {
    ...
    items.add(
      _HomeListItem(
        ...
        count: trashCount,
        folderId: '',
        trailingBadge: trashCount > 0 ? 'âš ï¸' : null,
      ),
    );
...
class _TopPillActions extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final bg = colors.isLight ? const Color(0xFFF2F2F7) : const Color(0xFF1C1C1E);
    final border = colors.separator.withValues(alpha: colors.isLight ? 0.35 : 0.22);
            ...
            Align(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(8),
                  border: Border.all(color: const Color(0xFFFFFFFF).withOpacity(0.18), width: 1),
                ),
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(CupertinoIcons.sparkles, color: Color(0xFFFFFFFF), size: 18),
          ),
          ...
          CupertinoButton(
            ...
            child: Container(
              ...
              alignment: Alignment.center,
              decoration: BoxDecoration(
                color: colors.isLight ? const Color(0xFFF2F2F7) : const Color(0xFF2C2C2E),
                borderRadius: BorderRadius.circular(999),
                border: Border.all(color: border),
              ),
              child: const Icon(CupertinoIcons.add, color: Color(0xFF34C759), size: 20),
            ),
          ),
        ],
      ),
            ...
            child: Row(
              children: [
                ...
                if (item.trailingBadge != null) ...[
                  ...
                  Text(item.trailingBadge!, style: const TextStyle(fontSize: 16)),
                ],
              ],
            ),
          ),
    ...
    return Opacity(
      ...
      child: CupertinoButton(
        ...
        child: Container(
          ...
          child: const Icon(CupertinoIcons.add, color: Color(0xFFFFFFFF), size: 28),
        ),
      ),
    );
  }
...
class _AppleEmptyState extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final title = tab == _HomeTab.trash ? 'Papelera vacÃ­a' : 'No hay planillas';
    final msg = tab == _HomeTab.trash
        ? 'Las planillas movidas a papelera aparecen acÃ¡ durante un tiempo para poder recuperarlas.'
        : 'CreÃ¡ tu primera hoja y empezÃ¡ a trabajar. Si usÃ¡s carpetas por mes, queda ordenado solo.';
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text(title,
                    style: TextStyle(color: pal.textPrimary, fontSize: 13, fontWeight: FontWeight.w900)),
...
class _AppleSheetRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final subtitle = tab == _HomeTab.trash
        ? '${meta.rows} filas Â· ${fmt(meta.updatedAt)} Â· vence en ${daysLeftInTrash ?? 0} dÃ­a(s)'
        : '${meta.rows} filas Â· ${fmt(meta.updatedAt)} Â· $folderName';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            tab == _HomeTab.trash
                ? '${meta.rows} filas Â· vence en ${daysLeftInTrash ?? 0} dÃ­a(s)'
                : '${meta.rows} filas Â· $folderName',
            style: TextStyle(color: colors.textSecondary, fontSize: 12, fontWeight: FontWeight.w600),
          ),
          if (note.isNotEmpty) ...[
            const SizedBox(height: 6),
      ...
      onPressed: () async {
        await showCupertinoModalPopup<void>(
          ...
          builder: (ctx) => CupertinoActionSheet(
            ...
            message: Text('$count planilla(s) Â· creada ${_fmtFolderCreated(folder.createdAtMs)}'),
            actions: [
              CupertinoActionSheetAction(
                onPressed: () {
                  Navigator.of(ctx).pop();
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(
                  '$count planilla(s) Â· creada ${_fmtFolderCreated(folder.createdAtMs)}',
                  style: TextStyle(color: pal.textSecondary, fontSize: 12, fontWeight: FontWeight.w600),
                ),
              ],
            ),
