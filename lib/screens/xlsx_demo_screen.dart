...
class _XlsxDemoScreenState extends State<XlsxDemoScreen> {
  ...
  Future<void> _sendEmail() async {
    ...
    await MailShare.sendFile(
      filePath:
          _lastPathOrUri ?? 'gridnote.xlsx', // en Web se usa como nombre lógico
      subject: subject,
      body: body,
    );
      ...
      : _rows = rows
            .map(
              (r) => DataGridRow(
                cells: [
                  DataGridCell<String>(
                      columnName: 'Fecha', value: _fmtDate(r.date)),
                  DataGridCell<String>(
                      columnName: 'Progresiva', value: r.progresiva),
                  DataGridCell<double>(
                      columnName: '3 electrodos', value: r.ohm3e),
                  DataGridCell<double>(
                      columnName: '4 electrodos', value: r.ohm4e),
                  DataGridCell<String>(
                      columnName: 'Observaciones', value: r.obs),
                ],
              ),
            )
            .toList();
