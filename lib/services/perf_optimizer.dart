...
class PerfOptimizer {
  ...
  /// Cola / backpressure.
  static _AsyncSemaphore _cpuSemaphore = _AsyncSemaphore(2); // default: 2 concurrentes
  ...
  static PerfStats _snapshot() {
    ...
    final avgMs = hist.isEmpty
        ? 0
        : (hist.reduce((a, b) => a + b) / hist.length).round();

    final p95Ms = _percentileMs(hist, 0.95);

    final jp = (_framesMeasured <= 0)
        ? 0.0
        : (_jankyFrames * 100.0) / _framesMeasured;
