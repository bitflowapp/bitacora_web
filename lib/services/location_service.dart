...
class LocationService {
  ...
  final Set<void Function(LocationFix)> _internalTaps = <void Function(LocationFix)>{};
  ...
  Future<void> _ensureServiceAndPermission() async {
    try {
      ...
      if (p == LocationPermission.deniedForever) {
        _emitEvent('permission_denied_forever_exception', {'perm': p.toString()});
        throw const LocationException(
          'Permiso denegado permanentemente. Revisá los ajustes de la app.',
        );
      }
  ...
  Future<void> _startSharedStream({
    ...
    try {
      ...
      _setStatus(LocationStatus.error(e, 'Sin permisos/servicio de ubicación.'));
    ...
    _sharedSub = Geolocator.getPositionStream(locationSettings: settings).listen(
          (p) {
        if (!_validPos(
          p,
          rejectMocked: rejectMocked,
  ...
  void _promoteCacheIfNeeded(LocationFix fix, {required String sourceForStatus}) {
    final now = DateTime.now();
