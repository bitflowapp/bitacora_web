...
double _computeAlpha(double rawAccuracy, _GpsFilterConfig cfg) {
  final accuracy =
  rawAccuracy <= 0 ? cfg.referenceAccuracyMeters : rawAccuracy;
...
class _GpsMapToolbarState extends State<GpsMapToolbar> {
  ...
  void _processPosition(Position position) {
    ...
    final accuracy = rawAccuracy <= 0
        ? _filterConfig.referenceAccuracyMeters
        : rawAccuracy;
  ...
  _GpsStatus _statusFromTracked(_TrackedPoint p) {
    final ageSeconds =
        DateTime.now().difference(p.timestamp).inSeconds;
  ...
  void _openFullScreenMap() {
    final trackedLatLng =
    _tracked == null ? null : LatLng(_tracked!.latitude, _tracked!.longitude);
          ...
          children: [
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 6),
                TextButton(
                  onPressed: coords.isEmpty ? null : _copyCoordinatesToClipboard,
